...
def get_extension_keys():
    """Выдает ключи браузерному расширению (с защитой от подделки)"""
    try:
        # Получаем информацию о клиенте
        ...
        # Логируем запрос
        logger.info(f"📦 Extension keys request: v{extension_version}, ID: {extension_id[:10]}..., IP: {request.remote_addr}")
        
        # ✅ БАЗОВАЯ ПРОВЕРКА БЕЗОПАСНОСТИ
        # 1. Проверяем User-Agent (должен быть Chrome Extension)
        if 'chrome-extension' not in user_agent.lower() and 'moz-extension' not in user_agent.lower():
            logger.warning(f"❌ Suspicious User-Agent for keys: {user_agent[:50]}")
            return jsonify({
                "error": "Unauthorized client",
                "message": "Only browser extensions can request keys"
            }), 403
        
        # 2. Проверяем версию расширения
        valid_versions = ['1.0', '1.1', '1.2']
        if extension_version not in valid_versions:
            logger.warning(f"❌ Invalid extension version: {extension_version}")
            return jsonify({
                "error": "Unsupported version",
                "message": f"Please update to one of: {', '.join(valid_versions)}"
            }), 403
        
        # 3. Rate limiting по IP (строже чем обычно)
        ip_key = f"key_request_{request.remote_addr}"
        requests_count = getattr(request, 'key_requests', 0)
        if requests_count > 10:  # Макс 10 запросов в час
            logger.warning(f"⛔ Rate limit exceeded for keys: {request.remote_addr}")
            return jsonify({
                "error": "Rate limit exceeded",
                "message": "Too many key requests"
            }), 429
        
        # ✅ ФОРМИРУЕМ ОТВЕТ С КЛЮЧАМИ
        keys_response = {
            "status": "success",
            "timestamp": datetime.now().isoformat(),
            "keys": {
                # Основные ключи для аутентификации
                "SECRET_KEY": SECRET_KEY,  # Берется из переменных окружения
                "HMAC_SECRET_KEY": HMAC_SECRET_KEY,  # Берется из переменных окружения
                
                # VirusTotal API ключ - ВАЖНО: можно ограничить или выдать временный
                "VIRUSTOTAL_API_KEY": VIRUSTOTAL_API_KEY if VIRUSTOTAL_API_KEY else "",
                
                # Дополнительная информация
                "SERVER_URL": "https://phishguard-server-production.up.railway.app",
                "EXPIRES_IN": 86400,  # 24 часа в секундах
            },
            "metadata": {
                "version": extension_version,
                "server_time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "refresh_interval": 24  # часов
            }
        }
        
        # ✅ ДОПОЛНИТЕЛЬНАЯ БЕЗОПАСНОСТЬ (опционально):
        # Если хотите еще больше безопасности, можно:
        # 1. Генерировать временные ключи
        # 2. Использовать JWT токены
        # 3. Вести белый список extension IDs
        
        logger.info(f"✅ Keys issued to extension v{extension_version} from {request.remote_addr}")
        
        # ✅ Отправляем в Telegram уведомление (если включено)
        if TELEGRAM_ENABLED and telegram_alerts and telegram_alerts.enabled:
            try:
                telegram_alerts.send_message(
                    f"🔑 *Extension Keys Issued*\n"
        ...
        return jsonify(keys_response)
        
    except Exception as e:
        logger.error(f"❌ Extension keys error: {e}")
        return jsonify({
...
def is_valid_extension(extension_id, version):
    ...
    В реальном проекте можно вести базу разрешенных extension IDs
    """
...
# ==================== SECURE KEY REFRESH ====================
@app.route('/api/secure/extension-keys', methods=['POST'])
@hmac_required  
@rate_limit
def get_secure_extension_keys():
    """Защищенный эндпоинт для выдачи ключей (требует HMAC)"""
    try:
        data = request.json or {}
        
        # Получаем данные из запроса
        extension_id = data.get('extension_id', '')
        public_key = data.get('public_key', '')  # Публичный ключ расширения
        platform = data.get('platform', 'chrome')
        
        # Логируем
        logger.info(f"🔐 Secure keys request from {extension_id[:10]}...")
        
        # Генерируем ключи для этого расширения
        # В реальном проекте можно генерировать уникальные ключи для каждого расширения
        response = {
            "status": "success",
            "keys": {
                "SECRET_KEY": SECRET_KEY,
        ...
        logger.error(f"Secure keys error: {e}")
